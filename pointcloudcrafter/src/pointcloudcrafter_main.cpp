...
int main(int argc, char * argv[])
{
   ...
   * Output control
   * ========================= */
  app.add_flag("--sequential-name", pointcloudcrafter::SEQUENTIAL_NAMES,
               "Use sequential file names instead of timestamps")
    ->group("Output");

  app.add_flag("--timestamps", pointcloudcrafter::TIMESTAMPS,
               "Save point cloud timestamps to a text file")
    ->group("Output");

  /* =========================
   * Frame selection
   * ========================= */
  app.add_option("-m,--max-frames", pointcloudcrafter::MAX_FRAMES,
                 "Maximum number of frames to extract (-1 = unlimited)")
    ->group("General");

  app.add_option("-j,--skip-frames", pointcloudcrafter::SKIP_FRAMES,
                 "Number of frames to skip at the beginning")
    ->group("General");

  app.add_option("-s,--stride-frames", pointcloudcrafter::STRIDE_FRAMES,
   ...
   * Transform options
   * ========================= */
  app.add_option("-t,--target-frame", pointcloudcrafter::TARGET_FRAME,
                 "Target TF frame for all point clouds")
    ->group("Transforms");

  app.add_option("--transform-file,--tf", pointcloudcrafter::TRANSFORM_FILE,
   ...
   * Filtering
   * ========================= */
  app.add_option("--crop-box,--cb", pointcloudcrafter::CROPBOX,
                 "Crop box [xmin ymin zmin xmax ymax zmax]")
    ->expected(6)
    ->type_name("FLOAT FLOAT FLOAT FLOAT FLOAT FLOAT")
    ->group("Filtering");

  app.add_option("--crop-sphere,--cs", pointcloudcrafter::CROPSPHERE,
                 "Crop to sphere with given radius")
    ->type_name("FLOAT")
    ->group("Filtering");

  app.add_option("--crop-cylinder,--cc", pointcloudcrafter::CROPCYLINDER,
                 "Crop to cylinder with given radius")
    ->type_name("FLOAT")
    ->group("Filtering");

  app.add_option("--voxel-filter,--vf", pointcloudcrafter::VOXELFILTER,
                 "Voxel size [x y z]")
    ->expected(3)
    ->type_name("FLOAT FLOAT FLOAT")
    ->group("Filtering");

  app.add_option("--outlier-radius-filter,--orf",
                 pointcloudcrafter::OUTLIERRADIUSFILTER,
                 "Radius outlier removal [radius min_neighbors]")
    ->expected(2)
    ->type_name("FLOAT INT")
    ->group("Filtering");

  app.add_option("--outlier-stat-filter,--osf",
                 pointcloudcrafter::OUTLIERSTATFILTER,
