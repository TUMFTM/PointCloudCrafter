...
PointCloudCrafter is implemented in C++ and is designed primarily as a command-line toolkit composed of standalone executables for point cloud processing. The software builds on the Point Cloud Library (PCL) [@Rusu2011], which provides the underlying data structures and algorithms for ROS 2 point cloud streaming and for commonly used file formats such as PCD. PointCloudCrafter does not reimplement PCL functionality; instead, it exposes selected operations and extends them through a unified, reproducible command-line interface.
...
PointCloudCrafter has been applied across diverse robotics domains to solve specific engineering problems. For example, researchers used it to create an automated pipeline that converts ROS 2 bag recordings into nuScenes- or KITTI-style datasets. These datasets are used to train and evaluate machine learning perception algorithms [@Tempfli2025] and to create a public dataset for a wide range of applications [@Kulmer2025-2]. In localization research, the tool was used to extract and compile point cloud datasets for LiDAR odometry benchmarking [@Sauerbeck2023]. In mapping, the tool was used for benchmarking and for creating and post-processing large-scale point cloud maps [@Leitenstern2025; @Kulmer2025-1]. Additionally, PointCloudCrafter facilitated the development of novel LiDAR calibration approaches by extracting and isolating specific point cloud objects from continuous ROS 2 recordings [@Tahiraj2024; @Tahiraj2026].
Beyond these projects, PointCloudCrafter is used for analyses such as inspecting sensor drivers for timestamp anomalies or packet losses that result in incomplete or irregular point clouds.

Although it is difficult to track the exact adoption of open-source utilities, PointCloudCrafter has been rigorously tested across domains such as calibration, localization, mapping, and perception.
...
The Python package is available at \url{https://pypi.org/project/pointcloudcrafter}. The published wheel targets Linux `x86_64` (`manylinux`) with CPython $\geq$ 3.12; users on other platforms, architectures, or Python versions should build from source or use the Docker image.
The Docker image is available on the GitHub Container Registry at \url{https://github.com/TUMFTM/PointCloudCrafter/pkgs/container/pointcloudcrafter} and can be pulled via `ghcr.io/tumftm/pointcloudcrafter:latest`.
